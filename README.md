...
Copy `.env.example` to `.env` and set Cloudinary variables (`CLOUDINARY_URL` or `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`). Optional: `MODELS_DIR` (folder containing `model_sub`, `model_top`, `model_bottom`, `model_shoes`), `UPLOAD_ROOT`, `ALLOW_ORIGINS`.

**Download trained models from Google Drive** (same four folders as [this Drive folder](https://drive.google.com/drive/folders/1cIID51--8h882p04Nv0VkBPuHlB804xq)): from the repo root run `python scripts/download_models.py` (uses `gdown` from `requirements.txt`). That fills `models-saved/models/`. You can instead download manually from Drive and copy `model_sub`, `model_top`, `model_bottom`, `model_shoes` into `models-saved/models/`.

**Desktop UI (Tkinter):** from the repo root run `python launch_app.py`, or in Python: `from py.ui_module import run_ui` then `run_ui()`. Trained models must live under `models-saved/models/` (or `MODELS_DIR`).

**HTTP API (FastAPI):** from the repo root run `uvicorn backend.main:app --reload --host 0.0.0.0 --port 8000`. Open `http://127.0.0.1:8000/docs`. For production (e.g. Railway), use host `0.0.0.0` and the platform `PORT` (see `Procfile`).

**Quick CLI demo:** `python main.py` (expects sample images under `pictures/`).
